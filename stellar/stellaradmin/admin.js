...
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlyenB1eGh2a3RwY3drc21sbndsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTYyOTY1NzksImV4cCI6MjA3MTg3MjU3OX0.rjUVbGsQvPsLaua936DqA9fB5CVq8puRTq6DgJ1L_bs"; // replace with your anon key
...
const BRANCHES = {
  ...
  CE: {
    subjects: [
      { code: "CE13101_2", name: "Mathematics III" },
      { code: "CE13102_2", name: "Structural Analysis" },
      { code: "CE13103_2", name: "Geotechnical Engineering" },
      { code: "CE13104_2", name: "Surveying" },
      { code: "CE13105_2", name: "Concrete Technology" },
      { code: "CE13201_2", name: "Surveying Lab" },
      { code: "CE13202_2", name: "Concrete Lab" }
    ],
    rollRange: [120, 142],
    exclude: [],
    extra: []
...
// When branch changes → render subjects
branchSelect.addEventListener("change", () => {
  ...
  subjectsBox.innerHTML = branch.subjects.map(s => `
...
// Generate roll numbers for a branch
function getRolls(branchKey) {
  ...
  return [...rolls, ...b.extra].sort(); // Sort the rolls
}

function selectedSubjectCodes() {
  return [...subjectsBox.querySelectorAll('input[type="checkbox"]:checked')].map(i => i.value);
}
...
function buildGrid() {
  ...
  attBody.innerHTML = currentRolls.map(r => `
    <tr data-roll="${r}">
  ...
  // Bulk toolbar to mark all absent
  bulkToolbar.innerHTML = activeCodes.map(c => `
    <div class="bulk">
  ...
  bulkToolbar.querySelectorAll("button[data-bulk-absent]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      attBody.querySelectorAll(`td[data-code="${code}"] .status-select`).forEach(s => { s.value = "Absent"; });
    });
  });
...
clearGridBtn.addEventListener("click", () => {
  subjectsBox.querySelectorAll('input[type="checkbox"]').forEach(c => (c.checked = false));
...
saveBtn.addEventListener("click", async () => {
  const when = attDate.value;
  const activeCodes = [...attHead.querySelectorAll("th[data-code]")].map(th => th.dataset.code);
  const allRollsInGrid = [...attBody.querySelectorAll("tr")].map(tr => tr.dataset.roll);
  ...
  // MODIFIED: Only collect students who are explicitly marked as "Present".
  const rowsToInsert = [];
  ...
  try {
    // MODIFIED: The delete operation now clears all records for the grid's students,
    // subjects, and date to ensure a clean slate before inserting the "Present" ones.
    ...
    // Now, insert only the "Present" records. If rowsToInsert is empty, this will do nothing.
    if (rowsToInsert.length > 0) {
      const { error: insErr } = await supabaseClient.from("attendance").insert(rowsToInsert);
      if (insErr) throw insErr;
    }

    saveStatus.textContent = `Done. Saved attendance for ${rowsToInsert.length} entries.`;
  } catch (e) {
    console.error(e);
    saveStatus.textContent = "Error while saving. Check the console.";
  } finally {
